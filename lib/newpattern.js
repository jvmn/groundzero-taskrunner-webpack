...
const templateVars = {
  ...
  outputDir: null,
  usingLocal: false,
  namespace: config.fractal.namespace
}
  ...
  return new Promise(function (resolve, reject) {
    try {
      inquirer.prompt([
        ...
        if (templateVars.projectPrefix) {
          templateVars.cssprefix = config.pattern.cssTypeAbbreviation ? templateVars.projectPrefix + templateVars.type.charAt(0) + '-' : templateVars.projectPrefix
        } else {
          templateVars.cssprefix = ''
        }
...
async function renderTemplate() {
  try {
    const file = await openTemplateFile('pattern.' + templateVars.engine)
    const proccessTemplate = await template(file, templateVars)
    saveFile(proccessTemplate, templateVars.engine)
  } catch (error) {
...
try {
  (async () => {
    templateVars.engine = config.pattern.engineExt || 'hbs'
    console.log(`Generating ${templateVars.engine} pattern...`)
